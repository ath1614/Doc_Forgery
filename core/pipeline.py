...
from core.forensics    import MultiScaleELA, CopyMoveDetector, NoiseInconsistencyDetector, PicsartEditDetector, DocumentTypeVerifier
...
def _extract_fields(text: str, doc_type: str) -> Dict[str, Any]:
    ...
    if doc_type == 'AADHAAR':
        ...
        # Extract name — skip header words like 'Permanent Account Number Card'
        _PAN_SKIP = {'Permanent', 'Account', 'Number', 'Income', 'Government', 'Department', 'India'}
        for nm in re.finditer(r'([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)', text):
            words = set(nm.group().split())
            if not words & _PAN_SKIP:
                fields['name'] = nm.group()
                break
...
class DocumentForgeryDetector:
    def __init__(self):
        ...
        self.doc_verifier  = DocumentTypeVerifier()
    ...
    def analyze_image(self, image_bytes: bytes,
        ...
        futures = {
            'ela':       self._executor.submit(self.ela.analyze,       image),
            'copy_move': self._executor.submit(self.copy_move.detect,  image),
            'noise':     self._executor.submit(self.noise.detect,      image),
            'font':      self._executor.submit(self.font.check,        image, text_blocks),
            'picsart':   self._executor.submit(self.picsart.detect,    image),
        }
        ...
        # ── 3b. Document type verification ──────────────────
        doc_verify_r = self.doc_verifier.verify(image, doc_type, full_text)
        signals['doc_verify'] = doc_verify_r.get('score', 0.0)
        evidence += self._ev('doc_type_mismatch', doc_verify_r.get('issues', []), doc_verify_r.get('score', 0.0))
        ...
        if selfie_bytes:
            try:
                ...
                face_match_result = self.face_matcher.match(image, selfie_img)
        ...
        if doc_type == 'AADHAAR':
            val        = self.aadhaar_val.validate(fields, image)
            val_score  = val['score']
    ...
    def analyze_pdf(self, pdf_bytes: bytes,
        ...
        try:
            ...
            doc.close()
            img_result = self.analyze_image(pix.tobytes('png'), selfie_bytes)
    ...
    def analyze(self, file_bytes: bytes, filename: str = '',
        ...
        if Path(filename).suffix.lower() == '.pdf':
            return self.analyze_pdf(file_bytes, selfie_bytes)
        return self.analyze_image(file_bytes, selfie_bytes)

    async def analyze_async(self, file_bytes: bytes, filename: str = '',
                            selfie_bytes: Optional[bytes] = None) -> Dict[str, Any]:
        return await asyncio.to_thread(self.analyze, file_bytes, filename, selfie_bytes)
